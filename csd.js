		...
		//Parameters
		var initialGuessMean = 0;
		var initialGuessSlope = 1;
		var initialGuessK = 1;
		var binSize = 0.01;
		
		var numberOfDataPoints = 20;
		var actualMean = 0;
		var actualSlope = 1;
		var noise = 0.0;
		var xRange = [-1,1];
		...
		//Step A: Record data
		[confidenceArray, stimulusArray] = [confidenceArray2, stimulusArray2];//makeConfidenceAndStimulusArrays(xRange, [actualMean, actualSlope]);
		...
		function makeBinary(confidenceArray){
			...
			for(var i = 0; i < confidenceArray.length; i++){
				...
				var binaryData = confidenceArray[i] > 0.5 ? 1 : 0;
